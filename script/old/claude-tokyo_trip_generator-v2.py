...
class TokygeneGenerator:
    ...
    def process_section_content(self, section_id: str, content: str) -> str:
        """Process เนื้อหาของแต่ละ section อย่างยืดหยุ่น"""
        # ดึง title และ content แยกกัน
        title_match = re.search(r'^#+\s*(.+?)(?:\n|$)', content, re.MULTILINE)
        title = title_match.group(1).strip() if title_match else f"Section {section_id.title()}"
        
        # ลบ title ออกจาก content (หา header ระดับแรกเท่านั้น)
        body_content = content
        if title_match:
            # ลบเฉพาะ header แรก
            header_end = title_match.end()
            body_content = content[header_end:].lstrip('\n')
        
        # แยก Thai/English title อย่างยืดหยุ่น
        thai_title = title
        english_title = title
        
        # ลองแยกด้วย separator ต่างๆ
        separators = [' - ', ' – ', ' | ', ' / ']
        ...
        # Process content based on type - ตรวจสอบแบบยืดหยุ่น
        if (re.search(r'^- \*\*\d+:\d+\*\*:', body_content, re.MULTILINE) or
            re.search(r'^- \*\*\d{1,2}:\d{2}\*\*:', body_content, re.MULTILINE)):
            # Timeline content
    ...
    def generate_html(self) -> str:
        ...
        # จัดเรียงและสร้าง sections อื่นๆ อย่างยืดหยุ่น
        # Section order ที่แนะนำ (ถ้ามี)
        preferred_order = [
            'weather-info', 'weather_info',
            'day1', 'day2', 'day3', 'day4', 'day5', 'day6', 'day7', 'day8',
            'budget', 'accommodation', 'timeline', 
            'food-recommendations', 'food_recommendations',
            'shopping-guide', 'shopping_guide',
            'kawaguchiko-guide', 'kawaguchiko_guide',
            'important-updates', 'important_updates',
            'transportation-budget', 'transportation_budget'
        ]
        
        # เพิ่ม sections ตาม preferred order
        ...
        # เพิ่ม sections ที่เหลือ (ยกเว้น overview, main-info, tokyo-trip-update)
        ...
        for section_id, content in markdown_contents.items():
            if (section_id not in processed_sections and 
                section_id not in skip_sections and
                content.strip()):
